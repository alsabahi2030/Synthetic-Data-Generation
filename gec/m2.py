...
def m2_to_parallel(m2_files, ori, cor, drop_unchanged_samples, all):
    ...
    ori_fout = None
    if ori is not None:
        ori_fout = open(ori, 'w',encoding='utf-8',errors='ignore')
    cor_fout = open(cor, 'w',encoding='utf-8',errors='ignore')

    # Do not apply edits with these error types
    skip = {"noop", "UNK", "Um"}
    for m2_file in m2_files:
        entries = open(m2_file,encoding='utf-8',errors='ignore').read().strip().split("\n\n")
...
def  sys_to_cor(system_out, cor_path,infer_boosting=False):
    ...
    lines = open(system_out, 'r',encoding='utf-8').read().strip().splitlines()
    hypo_lst = []
    """
    if infer_boosting:
        line_start = "BH"
    ...
    with open(cor_path, 'w', encoding='utf-8') as fout:
        fout.write("\n".join(detokenize(sent) for sent in hypo_lst))
        fout.write("\n")


def sys_to_cor_bpe(system_out, cor_path):
    ...
    hypo_lst = _sorted(hypo_lst)
    with open(cor_path, 'w', encoding='utf-8') as fout:
        fout.write("\n".join(sent.strip().replace('``','"').replace('\'\'','"') for sent in hypo_lst))
        fout.write("\n")
...
def get_m2_entries(m2_file):
    m2_entries = open(m2_file,encoding='utf-8').read().strip().split('\n\n')
    return m2_entries


def write_m2_entries(m2_entries, m2_file):
    with open(m2_file, 'w',encoding='utf-8') as fout:
        fout.write("\n\n".join(m2_entries))
...
def split_m2(fpaths, finetune_m2, valid_m2, finetune_split_ratio):
    ...
    """
    with open(finetune_m2, 'w',encoding='utf-8') as finetune_out, open(valid_m2, 'w') as valid_out:
        for fpath in fpaths:
            if 'ABCN' in fpath: continue
            lines = open(fpath, 'r',encoding='utf-8').read().split('\n\n')
            for line in lines:
