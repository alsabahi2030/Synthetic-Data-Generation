...
class NoiseInjector(object):
    ...
    def inject_noise(self, tokens):
        ...
        try:
            error_probs = [.25, .25, .25, .25]
            funcs = [self._add_func, self._shuffle_func, self._replace_func, self._delete_func]
            #np.random.shuffle(funcs)
            error_fun = npchoice([self._add_func, self._shuffle_func, self._replace_func, self._delete_func],
                                 p=error_probs)
            pairs = [(i, w) for (i, w) in enumerate(tokens)]
            pairs = error_fun(pairs)
            art, align = self._parse(pairs)
        except ValueError:
            print("Oops! There was a problem in this sentence {}".format(tokens))
            return None,None
        return art, align
...
if __name__ == '__main__':
    ...
    filename = "/container_data/src/helo_word-master/data/parallel/raw/wi.train.cor"
