...
def suggest(text, is_first, speller, n=NUM_OF_SUGGESTIONS):
    ...
    if text.isalpha():
        if is_upper:
            ...
            raw_suggestions = speller.lookup(text, Verbosity.TOP, max_edit_distance=2)
            for suggestion in raw_suggestions:
                suggestions.append(suggestion._term[0].upper() + suggestion._term[0][1:])

        else:
            raw_suggestions = speller.lookup(text, Verbosity.CLOSEST, max_edit_distance=2)

            for suggestion in raw_suggestions:
                if suggestion._term[0].isupper():
                    continue
                suggestions.append(suggestion._term)
...
def check(fin, fout,
    ...
    speller.load_dictionary(dic, term_index=0, count_index=1)
